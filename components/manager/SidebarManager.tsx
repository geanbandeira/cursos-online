  ...
  return (
    <aside className="hidden md:flex w-64 bg-[#00324F] text-white flex-col fixed inset-y-0 shadow-2xl z-50">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${isActive
      ...
      <div className="mt-auto p-6 border-t border-white/10 space-y-2">
        <Link
          href="/my-courses"
          className="flex items-center gap-3 px-4 py-3 text-slate-400 font-bold text-xs uppercase hover:text-white transition-all"
        >
        ...
        <button
          ...
          className="flex items-center gap-3 px-4 py-3 text-red-400 font-bold text-xs uppercase hover:text-red-300 transition-all w-full text-left cursor-pointer"
        >
